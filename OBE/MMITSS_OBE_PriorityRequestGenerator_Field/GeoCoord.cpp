...
void geoCoord::ecef2local(double ex, double ey, double ez, double *x, double *y, double *z)
{
	//compute the local (x,y,z) coordinates
	*x=(-R_sinLat*R_cosLong)*(ex-ex_init)+(-R_sinLat*R_sinLong)*(ey-ey_init)+R_cosLat*(ez-ez_init);
	*y=(-R_sinLong)*(ex-ex_init)+(R_cosLong)*(ey-ey_init)+0;
	*z=(-R_cosLat*R_cosLong)*(ex-ex_init)+(-R_cosLat*R_sinLong)*(ey-ey_init)+(-R_sinLat)*(ez-ez_init);
}
