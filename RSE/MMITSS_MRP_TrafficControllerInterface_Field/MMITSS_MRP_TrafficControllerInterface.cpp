...
int main ( int argc, char* argv[] )
{
	...
	{	
		...
		//~ if(count>=5)
		//~ {
			//~ //Set signal controller to free operation every 1 second
			//~ int setvalue=254;
			//~ Coord_Pattern_Set(setvalue);
			//~ count=0;
			//~ 
		//~ }
		...
		while(!Eventlist_R1.EndOfList())
		{
			if (currenttime-begintime<Eventlist_R1.Data().time)   //used as hold command or omit
			{
