...
#define	PHASE_HOLD 3
#define PHASE_PED_CALL 4   //Add by YF 04/27/2015
...
extern int occupancy[8];
extern int Ped_Info[2];
...
void IntersectionPedConfigRead(int CurTimingPlanNo,char *ConfigOutFile);  //Read configured ped walking time and clearance time
void Coord_Pattern_Set(int value);
    ...
    if (status == STAT_SUCCESS && response->errstat == SNMP_ERR_NOERROR)
    {
        ...
        */
        ...
        for(int i=0;i<8;i++)
        {
            ...
            cout<<"CurPhaseStatus "<<i+1<<" "<<CurPhaseStatus[i]<<endl;
...
void IntersectionPedConfigRead(int CurTimingPlanNo,char *ConfigOutFile)
{
    ...
    */
    ...
    {
        sprintf(ctemp,"%s%d",PED_WALK,i);  // Ped Walk time, only read for plan 1 NTCIP standard command
    ...
    for(int i=1;i<=8;i++)
    {
        sprintf(ctemp,"%s%d",PED_CLEAR,i);  // Ped Clearance time , only read for plan 1 NTCIP standard command
        if (!snmp_parse_oid(ctemp, anOID, &anOID_len)) 
        {
            snmp_perror(ctemp);
            SOCK_CLEANUP;
    ...
    */
    if (status == STAT_SUCCESS && response->errstat == SNMP_ERR_NOERROR)
    {
        ...
        */
        ...
        int Result[3][8];  //*** Sequence: MinGrn[8],Yellow[8],RedClr[8],GrnMax[8],PhaseSeq[8],PedWalk[8],PedClr[8];***//  Yiheng added for row 6 and 7 for ped information 10/24/2014
        /*
        Result[0][8]  ----> MinGrn[8]
        Result[1][8]  ----> Yellow[8]
        Result[2][8]  ----> RedClr[8]
        ...
        */
        ...
        fs_config<<"PedWalk";
        for(int i=0;i<8;i++) fs_config<<"\t"<<PedWalk[i];
        fs_config<<endl;
        
		fs_config<<"PedClr";
        for(int i=0;i<8;i++) fs_config<<"\t"<<PedClr[i];
        fs_config<<endl;
...
void IntersectionPhaseControl(int phase_control, int Total,char YES)
{
    ...
    */
	...
	else if (PHASE_PED_CALL==phase_control)
	{
		if (!snmp_parse_oid(MIB_PHASE_PED_CALL, anOID, &anOID_len))
		{
			snmp_perror(MIB_PHASE_PED_CALL);
			failures++;
		}
		sprintf(tmp_log,"PED CALL to ASC controller! Number (%d), AT time:%ld \n ",Total,time(NULL));
        std::cout <<tmp_log;        //outputlog(tmp_log);
	}
...
void DetRead()
{
	...
	if (status == STAT_SUCCESS && response->errstat == SNMP_ERR_NOERROR)
	{
		...
		//cout<<"\n"<<endl;
		//cout<<"The VOLUME output of the detector information is: "<<out[0]<<" "<<out[1]<<" "<<out[2]<<" "<<out[3]<<" "<<out[4]<<" "<<out[5]<<" "<<out[6]<<" "<<out[7]<<endl;
		//logfile<<out[0]<<" "<<out[1]<<" "<<out[2]<<" "<<out[3]<<" "<<out[4]<<" "<<out[5]<<" "<<out[6]<<" "<<out[7]<<endl;
		for (int j=0;j<8;j++)
		{
			occupancy[j]=out[j];
		}
