...
char GeoFence_Range_file[64] = "/nojournal/bin/GeoFence_Range.txt";
...
void FindVehInMap(double Speed, double Heading,int nFrame,double N_Offset1,double E_Offset1,double N_Offset2,double E_Offset2, MAP NewMap, int pre_app, double &Dis_curr, double &est_TT, int &request_phase, int &approach, int &lane);
...
float GeoFence_Range[8]; 	 //Range of the GeoFence on each leg of the intersection
int distance_nearest_line; //The distance to the nearest line to be considered out of GeoFence
...
void get_GeoFence_Range(); //Shayan added 10.7.14

void printbsmcsv(struct bsm *bsm);
...
//}		
	...
	get_GeoFence_Range(); //call the function to read the GeoFence ranges and distance to the nearest line
...
//	memset(recvaddr.sin_zero,'\0',sizeof recvaddr.sin_zero);
	...
	while (true)
	{
		...
		{	
			...
			if (j2735_decode_bsm (&bsm, buf, sizeof(buf), &type, &oob) < 0) 
			{ 
				fprintf (stderr, "BSM Decode error\n");
				traj_requested=1;
			}
			else {
				traj_requested=0;
		...
		if(traj_requested==0)
		{
			...
			while(!trackedveh.EndOfList())  //match vehicle according to vehicle ID
				{
					...
					if(trackedveh.Data().TempID==bsm.temp_id)  //every 0.5s processes a trajectory 
					{
						...
						if (t1-trackedveh.Data().receive_timer>0.5)
						{
							...
							trackedveh.Data().traj[trackedveh.Data().nFrame].latitude=bsm.latitude;
							trackedveh.Data().traj[trackedveh.Data().nFrame].longitude=bsm.longitude;
							trackedveh.Data().traj[trackedveh.Data().nFrame].elevation=bsm.elevation;
							//change GPS points to local points
							geoPoint.lla2ecef(bsm.longitude,bsm.latitude,bsm.elevation,&ecef_x,&ecef_y,&ecef_z);
							geoPoint.ecef2local(ecef_x,ecef_y,ecef_z,&local_x,&local_y,&local_z);
							trackedveh.Data().N_Offset[trackedveh.Data().nFrame]=local_x;
							trackedveh.Data().E_Offset[trackedveh.Data().nFrame]=local_y;
							trackedveh.Data().acceleration=(bsm.speed-trackedveh.Data().Speed)*2;		//instantaneously acceleration added 4/20/2014					
							trackedveh.Data().Speed=bsm.speed/speed_coif;       //Speed from Savari BSM decoder is KM/H, need to change to m/s here 
							trackedveh.Data().heading=bsm.heading;
							//cout<<"Veh heading is: "<<bsm.heading<<endl;
							trackedveh.Data().Dsecond=bsm.secmark;
							...
							if(trackedveh.Data().Speed<0.8 && trackedveh.Data().stop_flag==0)  //vehicle has not stopped before, for the ELVS algorithm
							{
								trackedveh.Data().stop_flag=1;
								trackedveh.Data().queue_flag=1; //for Performance Obserevr
								trackedveh.Data().time_stop=GetSeconds();
								//sprintf(temp_log,"Veh ID is: %d and Time stop is %lf\n",trackedveh.Data().TempID,trackedveh.Data().time_stop);
								//outputlog(temp_log); cout<<temp_log;
								//cout<<"Veh id is: "<<trackedveh.Data().TempID<<" and "<<"Time stop is: "<<trackedveh.Data().time_stop<<endl;
							}
							
							if(trackedveh.Data().Speed>0.8 && trackedveh.Data().stop_flag==1)  // Added by Shayan to keep track of the last stopped vehicle 
							{
							...
							if (flag_field==1)  //in the field log the received BSM data
							{
								//gps_init();
								//savari_gps_read(&gps, gps_handle);
								sprintf(temp_log,"Timestamp %.2lf: %d %lf %lf %f %f %.1lf\n",GetSeconds(),trackedveh.Data().TempID,
								trackedveh.Data().traj[trackedveh.Data().nFrame].latitude,trackedveh.Data().traj[trackedveh.Data().nFrame].longitude,trackedveh.Data().Speed,trackedveh.Data().heading,GetSeconds());								
			...
			if(found==0)  //this is a new vehicle
				{
					TempVeh.TempID=bsm.temp_id;
					TempVeh.traj[0].latitude=bsm.latitude;
					TempVeh.traj[0].longitude=bsm.longitude;
					TempVeh.traj[0].elevation=bsm.elevation;
					//change GPS points to local points
					geoPoint.lla2ecef(bsm.longitude,bsm.latitude,bsm.elevation,&ecef_x,&ecef_y,&ecef_z);
					geoPoint.ecef2local(ecef_x,ecef_y,ecef_z,&local_x,&local_y,&local_z);
					TempVeh.N_Offset[0]=local_x;
					TempVeh.E_Offset[0]=local_y;
					TempVeh.Speed=bsm.speed/speed_coif;
					...
					if(Tempdis<1000000.0)  // Find a valid node
					{
						...
						{
							DistanceFromLine(local_x, local_y, MAP_Nodes[N_pos].N_Offset, MAP_Nodes[N_pos].E_Offset , MAP_Nodes[N_pos+1].N_Offset, MAP_Nodes[N_pos+1].E_Offset, distanceSegment, distanceLine);
						...
						dis_to_ref = sqrt(pow(local_x,2) + pow(local_y,2));
								...
								{	
									...
									sprintf(temp_log,"Vehicle No. %d is outside the GeoFence with location of %lf %lf\n",TempVeh.TempID,TempVeh.traj[0].latitude,TempVeh.traj[0].longitude);
									outputlog(temp_log);
								}
					}
					else
	...
	//-----------------------------------Vehicle Exit form the Geo Fenced Area-------------------------------//
			...
			if (Tempdis<1000000.0)
			{
				...
				dis_to_ref = sqrt(pow(tmp_N_Offset1,2) + pow(tmp_E_Offset1,2));
						...
						if (dis_to_ref >= GeoFence_Range[N_App-1] || distanceLine >= distance_nearest_line)  //Outside the GeoFence Area
						{
						...
						else    
						{
							...
							if(trackedveh.Data().req_phase < 0 && dis_to_ref < 35) // if the vehicle passes the stopbar, set the approach and lane to be previous approach and lane
							{
								...
								trackedveh.Data().active_flag =14; // Just to make sure that the vehicle is not deleted in this situation!
							...
							else //if(trackedveh.Data().req_phase >= 0 || dis_to_ref >= 35) //Inside the GeoFence but not within the intersection
							{
								...
								FindVehInMap(tmp_speed,tmp_heading,tmp_nFrame,tmp_N_Offset1,tmp_E_Offset1,tmp_N_Offset2,tmp_E_Offset2,NewMap,pre_app,Dis_curr,ETA,requested_phase,T_approach,T_lane);		
								...
								if(trackedveh.Data().req_phase < 0 && dis_to_ref < 35) //For the first times after passimg the stop bar ignore what FindVehInMap did, because it was not correct!
								{
									trackedveh.Data().approach=trackedveh.Data().previous_approach;
									trackedveh.Data().lane=trackedveh.Data().previous_lane;
								}
								  
								//sprintf(temp_log,"AFTER Vehicle No. %d Approach %d Lane %d trackedveh.Data().req_phase %d dis_to_ref %f\n",trackedveh.Data().TempID,T_approach,T_lane,trackedveh.Data().req_phase, dis_to_ref);
								//cout<<temp_log;
								//outputlog(temp_log);
...
void FindVehInMap(double Speed, double Heading,int nFrame,double N_Offset1,double E_Offset1,double N_Offset2,double E_Offset2, MAP NewMap, int pre_app, double &Dis_curr, double &est_TT, int &request_phase,int &approach, int &lane)
{
				...
				if(nFrame>=2) //start from second frame
			{
				...
				if( abs(veh_heading - lane_heading)> 120 && abs(veh_heading - lane_heading) <240) // if GPS drifted and we locate the vefhicle in the egress approach instead of ingress approach, we should recover the correct approach and lane!!
				{
...
void PackTrajData(byte* tmp_traj_data,int &size)
{
	...
	while(!trackedveh.EndOfList())
	{
		if (trackedveh.Data().processed==1)
		{
			...
			//vehicle req_phase
			tempUShort = (unsigned short)trackedveh.Data().req_phase;
			pByte = (byte* ) &tempUShort;
			tmp_traj_data[offset+0] = (byte) *(pByte + 1); 
			tmp_traj_data[offset+1] = (byte) *(pByte + 0); 
			offset = offset + 2;
...
void get_GeoFence_Range() //Shayan Added 10.7.14
{
	...
	if(temp_string.size()!=0)
    {
		char tmp[128];
		strcpy(tmp,temp_string.c_str());		
		sscanf(tmp,"%f %f %f %f %f %f %f %f",&GeoFence_Range[0],&GeoFence_Range[1],&GeoFence_Range[2],&GeoFence_Range[3],&GeoFence_Range[4],&GeoFence_Range[5],&GeoFence_Range[6],&GeoFence_Range[7]);
    }
    else
    {
        sprintf(temp,"Reading GeoFence_Range_File problem!!");
        cout<<temp<<endl;
        outputlog(temp);
        exit(0);
    }
    
    getline(sh,temp_string);  //Reading the third line which explains the distance to nearest line
	getline(sh,temp_string);  //Reading the fourth line which has the value for this distance
	
	if(temp_string.size()!=0)
    {
		char tmp[128];
		strcpy(tmp,temp_string.c_str());		
		sscanf(tmp,"%d",&distance_nearest_line);
    }
    else
    {
        sprintf(temp,"Reading distance attribute problem!!");
        cout<<temp<<endl;
        outputlog(temp);
        exit(0);
    }
...
void printbsmcsv (struct bsm *bsm) {
	...
	if (first) {
		printf ("Timestamp,MsgID,MsgCnt,TempID,SecMark,Latt,Long,Elev,Speed,Heading\n");
		first = 0;
		//gps_init();
	}
	...
	//printf("%lf,", gps.time);
	printf("%x,", bsm->msgid);
	printf("%x,", bsm->msgcount);
	printf("%llx,", bsm->temp_id);
	printf("%d,", bsm->secmark);
	printf("%f,", bsm->latitude);
	printf("%f,", bsm->longitude);
	printf("%f,", bsm->elevation);
	printf("%f,", bsm->speed);
	printf("%f\n", bsm->heading);
}
